...
> **2026-05-14 Round-1-Update.** Anna's feedback (`feedback/round1/`) brought
> three structural changes that supersede parts of this brief:
>
> 1. **No more OpenAI API path.** The tool makes ZERO LLM calls of its own
>    (Phase 6). Codex (her ChatGPT account) writes the interpretation prose;
>    `uv run python -m ld.run` only does deterministic work. The Fallback
>    flow section below is historical context — it describes how things used
>    to work, not how they work today.
> 2. **5-page A4-landscape branded report.** Cover, athlete data, analysis
>    tables, interpretation, internal coach page — see `templates/report.docx`
>    generated by `build_report_template.py` (Phases 4+5).
> 3. **Output basename normalization** to `XY_LD_Sportart_JJ_MM_TT` (Phase 7).
>    Input filename is no longer the source of truth.
>
> All eight feedback phases landed; full plan lives in `feedback/round1/`.
> The build sequence M1–M8 below is the original roadmap, kept for context.
